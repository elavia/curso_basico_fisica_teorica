...
no hay relación lineal entre \vb{B} y \vb{H}, luego
\[
...
\]
Para un imán es \vb{J}$_L=0$ de manera que
\[
	\rotorm{H} = 0 \qquad \Rightarrow \qquad \vb{H} = -\Nabla\phi_m,
\]
que es un potencial escalar magnético, que explica \vb{H}.
Fuera del imań no hay $ \vb{J}_M $ y se tiene
\[
	\rotorm{B} = \frac{4\pi}{c}\vb{J}_T = 0.
\]
Se podría utilizar un potencial para \vb{B} por lo anterior pero esto fallará en la
...
\begin{figure}[htb]
	\begin{center}
	\includegraphics[width=0.8\textwidth]{images/fig_ft1_magnetizaciones.pdf}	 
	\end{center}
	\caption{}
\end{figure}
...
\]
y podemos eliminar \vb{M} de acuerdo con
\[
...
\[
	\vb{B}_I + 2 \vb{H}_I = 0.
\]
El campo \vb{H} se conoce como campo desmagnetizante.
...
Si el \vb{M} es paralelo a la superficie el imán será tanto más estable. En la figura bajo

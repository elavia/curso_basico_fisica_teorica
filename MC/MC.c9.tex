...
\subsection{Transformación canónica infinitesimal}
...
\subsection{Potencial electromagnético}
...
Veamos una traslación rígida ahora. Sea coordenada que representa traslación rígida
\[
	q \longrightarrow q + \delta q \qquad \vb{r}_i \longrightarrow \vb{r}_i + \delta q \hat{n}
\]
\[
	p = \dpar{T}{\dot{q}} = \sum_i^N \frac{m_i}{2}\left( \dpar{V_i}{q}\right)^2 =
	\sum_i^N m_i \vb{v}_i \dpar{\vb{v}_i}{\dot{q}} = \sum_i^N m_i \vb{v}_i \hat{n} = \vb{p}\hat{n}
\]
usándose que $dv^2_i = 2 v_i dv_i$ y
\[
	\dpar{\vb{v}_i}{\dot{q}} = \dpar{\vb{r}_i}{q} = \frac{\vb{r}_i+\delta q \hat{n} -\vb{r}_i}{\delta q} = \hat{n}.
\]

Si la coordenada $q$ es la asociada a la traslación rígida entonces el momento canónicamente conjugado
es el momento proyectado en esa dirección. Para fuerzas
\[
	Q_j = \sum_i^N \vb{F}^a_i \dpar{\vb{r}_i}{q_j} \qquad \Longrightarrow \qquad 
	Q = \left( \sum_i^N \vb{F}^a_i \right)\cdot \hat{n} = \vb{F}\cdot \hat{n}
\]
\[
	\frac{d}{dt}\left( \dpar{T}{\dot{q}_j} \right) - \dpar{T}{q_j} = Q_j
\]

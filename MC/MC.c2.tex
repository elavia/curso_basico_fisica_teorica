...
\label{ej1}
\item{ \bf }
Se tiene el sistema de la figura, donde x1 , x2 se miden a partir de las posiciones de
equilibrio. Sea q1 = x1 + x2 y q2 = x1 − x2 .
\begin{enumerate}[label=(\alph*)]
 \item Definen (q1 , q2 ) un conjunto admisible de coordenadas generalizadas?.
 \item Si q1 = 0, describa cualitativamente el movimiento de cada partı́cula. Idem si
q2 = 0.
 \item Calcular las fuerzas generalizadas Q1 y Q2.
\end{enumerate}


\label{ej2}
\item{ \bf }
Para los casos siguientes. Cuántos grados de libertad tiene el sistema?. Proponga
conjuntos de coordenadas generalizadas adecuadas:
\begin{enumerate}[label=(\alph*)]
 \item m1 y m2 se mueven en el plano de la mesa.
 \item Idem, pero la mesa rota con ω = cte..
 \item m1 y m2 se hallan dentro de un tubo. Si q1 y q2 se miden a partir del centro de
masa, son coordenadas apropiadas?.
 \item Las dos masas se hallan unidas entre sı́ por una barra rı́gida. Analice el caso en
que sólo pueden moverse horizontalmente y también el caso bidimensional.
 \item Discuta los casos P fijo y P móvil.
 \item Una masa enhebrada en un alambre elı́ptico.
 \item Una máquina de Atwood. Analice los casos en que la cuerda desliza y no desliza
sobre la polea.
 \item Una partı́cula puntual que cae por una esfera, con gravedad.
\end{enumerate}

\label{ej3}
\item{ \bf }
D1 y D2 son dos plataformas rotantes como se muestra en la figura. D1 se mueve
respecto a la tierra con velocidad θ̇1 . D2 se mueve respecto a D1 con velocidad θ̇2 .
Una partı́cula de masa m se mueve libremente sobre D2 . Escriba el lagrangiano del
sistema en términos de coordenadas polares ρ, φ, de un sistema cartesiano fijo a D2 .
Halle las ecuaciones de movimiento de la partı́cula e interprete.


\label{ej4}
\item{ \bf }
Se tiene el sistema de la figura. Hallar la aceleración de cada masa utilizando:
\begin{enumerate}[label=(\alph*)]
 \item Las ecuaciones de Newton y condiciones cinemáticas.
 \item Principio de los Trabajos Virtuales (PTV)
 \item Las ecuaciones de Lagrange.
 \item *Repita a y b, pero ahora considerando que las poleas tienen masa M y radio R.
\end{enumerate}

\label{ej5}
\item{ \bf }
Dos partı́culas de masa m1 y m2 están unidas por un hilo inextensible de longitud l;
m1 se mueve sólo sobre el eje x y m2 sólo sobre el y. Las condiciones iniciales son las
que indica la figura.
\begin{enumerate}[label=(\alph*)]
 \item Halle la ecuación de movimiento para θ utilizando el PTV.
 \item Halle la ecuación de Lagrage para θ.
 \item Si m1 = m2 ≡ m, halle la tensión T en el hilo como función de θ.
 \item Cuál es el perı́odo de movimiento de θ en este caso?. Suponga que θ sólo puede
...
\label{ej6}
\item{ \bf }
Dos partı́culas de masas m1 y m2 están unidas por un hilo como indica la figura. m1
se mueve en el plano de la mesa y m2 sólo verticalmente. En t = 0, m1 se encuentra
a una distancia r0 del orificio y se le aplica una velocidad v0 perpendicular al hilo.
\begin{enumerate}[label=(\alph*)]
 \item Escriba las ecuaciones de Lagrange y halle sus integrales primeras en términos
de las condiciones iniciales.
 \item Halle la tensión del hilo.
 \item Repita a y b suponiendo ahora que el movimiento de m2 es bidimensional.
\end{enumerate}

\label{ej7}
\item{ \bf }
Bajo la acción de la gravedad, una partı́cula de masa m se desliza por una superficie cónica ρ = z tgα, sin rozamiento.
\begin{enumerate}[label=(\alph*)]
 \item Halle las ecuaciones de movimiento de la partı́cula utilizando como coordenadas
generalizadas el ángulo θ medido en el plano perpendicular al eje del cono y la distancia r al vértice del mismo, tomada a lo largo del cono.
 \item Hallar r máximo y r mı́nimo para el caso en que α = 30◦ y las condiciones iniciales sean r(0) = a, ṙ(0) = 0, θ̇2 (0) = 4 3g/a.
 \item Halle el potencial efectivo unidimensional equivalente. Muestre que las órbitas
circulares son posibles y halle la velocidad de la partı́cula en tales órbitas.
 \item Suponiendo la partı́cula en movimiento circular, halle la constante del oscilador
y el perı́odo de oscilación para pequeñas perturbaciones de este movimiento.
Compare este perı́odo con el de revolución para hacer una descripción cualitativa
del movimiento perturbado.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
 \item Dado un sistema constituı́do por N partı́culas, cuál es el número de grados de
libertad del mismo? y cuál el de ecuaciones de vı́nculo?
\item Se puede utilizar una velocidad como coordenada generalizada?
\item Las fuerzas generalizadas se aplican sobre cada partı́cula?
\item El número de grados de libertad de un sistema, es independiente del sistema de
referencia utilizado para describir el movimiento?.
\item Para estudiar el equilibrio de un sistema, es siempre válido utilizar el principio
de los trabajos virtuales?.
\item Es válida la formulación lagrangiana para un potencial dependiente de la veloci-
dad? y para el campo electromagnético?.
\item Dé un ejemplo en que un desplazamiento virtual difiera de uno real. En qué
casos son iguales?.
\item Las ecuaciones de vı́nculo para un sistema fı́sico, dependen del sistema de refe-
rencia utilizado?, y las fuerzas de vı́nculo?.
\item Para calcular las fuerzas de vı́nculo de un sistema, qué métodos es posible em-
plear?.
\item Siempre se pueden escribir las ecuaciones de Newton desde el centro de masa de
un sistema?.
\item Para un sistema de N partı́culas, cuántas ecuaciones de Newton se necesitan? y
de Lagrange?.
\item Qué se entiende por un sistema inercial? Serán correctas las ecuaciones de
movimiento si se escribe el lagrangiano desde un sistema no inercial?
\item Para una carga en un campo electromagnético, se puede conservar el impulso
lineal de la misma? Qué magnitud se conserva?.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
 \item Halle las ecuaciones de movimiento utilizando el método de Lagrange.
 \item Para el caso g = 0, integre las ecuaciones para condiciones iniciales r(0) = r0, ṙ(0) = 0.
 \item Discuta el caso en que φ varı́a libremente.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
 \item Encuentre las ecuaciones de movimiento para el péndulo doble que oscila en un
plano.
 \item Halle una expresión aproximada de las mismas para pequeñas oscilaciones alrede-
dor de la posición de equilibrio estable.
 \item Resuelva las ecuaciones proponiendo una solución de tipo armónico para los
grados de libertad. En t = 0 ambas masas se hallan en reposo sobre la vertical
y a la inferior se le aplica una velocidad v0 perpendicular al hilo.
 \item Halle las tensiones sobre los hilos.
\end{enumerate}

\label{ej11}
\item{ \bf }
Una partı́cula de masa m se desliza sin fricción por un alambre fijo en el punto A y que forma un ángulo θ0 con un eje vertical y que se encuentra rotando alrededor del mismo eje con velocidad angular constante ω.
\begin{enumerate}[label=(\alph*)]
 \item Encuentre el lagrangiano y las ecuaciones de Lagrange.
 \item Halle r(t) sabiendo que a t = 0, r(0) = r0 , ṙ(0) = 0.
\end{enumerate}

\label{ej12}
\item{ \bf }
Considere el péndulo en tres dimensiones —péndulo esférico.
\begin{enumerate}[label=(\alph*)]
\item Encontrar las ecuaciones de Lagrange para el mismo.
\item A partir de las ecuaciones de Lagrange hallar las constantes de movimiento.
\item Discuta cualitativamente el movimiento de este péndulo.
\end{enumerate}

\label{ej13}
\item{ \bf }
Escriba el lagrangiano de un péndulo plano donde el punto de suspensión:
\begin{enumerate}[label=(\alph*)]
\item se desplaza uniformemente por un cı́rculo vertical de radio a con frecuencia ω,
\item efectúa oscilaciones verticales de la forma a cos ωt,
\item efectúa oscilaciones horizontales de la forma a cos ωt.
\end{enumerate}

\label{ej14}
\item{ \bf }
Encuentre el lagrangiano de los sistemas de la figura. Existe gravedad.

\label{ej15}
\item{ \bf }
Sea una partı́cula libre de masa m y carga q en un campo electromagnético con po-
tenciales φ, A (E = −∇φ − c−1 ∂A/∂t; B = ∇×A). Obtenga a partir del lagrangiano
L = T − U —donde U es un potencial generalizado dependiente de la velocidad— las
ecuaciones de movimiento. Muestre que la fuerza aplicada sobre la partı́cula es la de
Lorentz F = q(E + c−1 v×B), U = qφ − qc−1 v·A.

\label{ej16}
\item{ \bf }
Sean (x1 , y1 ), (x2 , y2 ), dos sistemas de referencia cartesianos bidimensionales. Suponga
que el origen de coordenadas O se mueve con v = cte. respecto a x1 , y1 y que los ejes
x2 , y2 rotan con velocidad angular constante. Hallar explı́citamente las ecuaciones de
transformación: x1 = x1 (x2 , y2 , t) y y1 = y1 (x2 , y2 , t).


\label{ej17}
\item{ \bf }
Encuentre el lagrangiano y las ecuaciones de movimiento del siguiente sistema: un
péndulo simple de masa m2 , con una masa m1 en el punto sostén, la cual puede
moverse sobre una lı́nea horizontal contenida en el plano de movimiento de m2 . Re-
suelva las ecuaciones de movimiento y halle la frecuencia de oscilación del sistema para
pequeños apartamientos de la posición de equilibrio estable. Suponga condiciones ini-
ciales adecuadas.

\label{ej18}
\item{ \bf }
Escriba el lagrangiano y las ecuaciones de movimiento del siguiente sistema: una
máquina de Atwood con una cuerda de largo l, una polea con momento de inercia
I y que rueda sin deslizar con la cuerda.

\label{ej19}
\item{ \bf }
Sea una partı́cula de masa m y carga q inmersa en un campo magnético uniforme
B = B0 zb.
\begin{enumerate}[label=(\alph*)]
 \item Si A = Bxb y — compruebe que B = ∇×A —, calcule las ecuaciones de movi-
miento y muestre que las órbitas son espirales cilı́ndricas. Calcule el radio y el
centro de la circunferencia transversal a dicha espiral. Las condiciones iniciales
son r(0) = (x0 , y0 , z0 ), v(0) = (ẋ0 , ẏ0 , ż0 ).
 \item Repita el punto a. pero ahora para el potencial vector A0 = 21 B×r.
 \item Calcule la función ψ que da el cambio de medida —cambio de gauge— A0 =
A + ∇ψ.
 \item Si v(0) = 0, interprete fı́sicamente la solución hallada en a.
\end{enumerate}


\label{ej20}
\item{ \bf }
Sea un oscilador isótropo bidimensional (kx = ky ≡ k).
\begin{enumerate}[label=(\alph*)]
 \item Escriba el lagrangiano del sistema y halle las ecuaciones de movimiento para las
coordenadas generalizadas q1 = x y q2 = y.
 \item Sea L∗ = mẋẏ − kxy. Halle las ecuaciones de movimiento para este sistema.
Compare con las obtenidas en a.
\end{enumerate}

\end{ejercicios}

Los siguientes ejercicios corresponden a principios variacionales, que no tiene entidad aún en el curso.

\begin{ejercicios}

\label{ej1}
\item{ \bf }
Suponga que sabe experimentalmente que una partı́cula cae una distancia dada y en un p tiempo td = 2yd /g, pero que no se conoce el tiempo de caı́da para otras distancias. Suponga además que el lagrangiano del problema se conoce pero en lugar de resolver la ecuación de movimiento se prueba una forma funcional y = a + bt + ct2 . Muestre R que la integral I = L dt resulta un extremo para valores reales de los coeficientes sólo cuando a = 0, b = 0 y c = −g/2.

\label{ej2}
\item{ \bf }
Una partı́cula se encuentra sometida a un potencial de tipo gravitatorio, V (r) = −k/r.
Suponiendo que las órbitas son circulares, encuentre la relación entre los perı́odos y
los radios de las órbitas utilizando el principio variacional de Hamilton †.
† Ayuda: considere la ecuación paramétrica de la elipse x(τ ) = a cos ατ , y(τ ) = b senατ , y
apartamientos de la trayectoria circular variando a ó b.

\label{ej3}
\item{ \bf }
Se tiene una partı́cula sometida solamente a un potencial del tipo oscilador armónico,
V (x) = 12 kx2 . Encuentre la ecuación de movimiento minimizando la acción de la
siguiente manera:
\begin{enumerate}[label=(\alph*)]
\item Divida el intervalo de integración en n partes.
\item Reemplace las derivadas por su valor medio ẋi = ∆xi /∆ti y la integral por la
sumatoria.
\item Imponga la condición de extremo ∂I/∂xi = 0, i = 0, 1, 2, . . . , n − 1.
\item Tome el lı́mite para ∆ti → 0, n → ∞.
\item Interprete el resultado.
\end{enumerate}

\label{ej4}
\item{ \bf }
Considere el movimiento unidimensional de una partı́cula de masa m sometida al
potencial V (x) = 12 kx2 + 14 βx4 (oscilador anarmónico). La solución de la ecuación
de movimiento no se conoce, pero, debido a la forma del potencial, se sabe que el
movimiento será periódico y se prueba con una serie de Fourier de la forma:
$ x(t) = \sum_{n=0}^\infty X an cos(nωt) $
(tomando t=0 en un punto de retorno) donde ω resulta la pulsación (desconocida) del
movimiento. Considere la integral de la acción entre dos puntos t1 y t2 separados por
el perı́odo T = 2π/ω.
\begin{enumerate}[label=(\alph*)]
\item Muestre que para q β = 0, puede encontrarse un j tal que a0 = 0, an = 0 para todo k representa una solución. Observar que todo j representra la n 6= j y ω = 1j m misma solución. Interprete este hecho. R
\end{enumerate}

\label{ej5}
\item{ \bf }
Según Fermat, la luz sigue una trayectoria que hace extrema la integral 12 n(x, y) ds,
donde n(x, y) es el ı́ndice de refracción del medio inhomogéneo que la luz atraviesa.
El problema se supone plano. Muestre que si n(x, y) = n0 (1 + y/h), donde n0 y h son
constantes, la trayectoria de la luz está dada por y = −h + (αh/n0 ) cosh(β + n0 x/hα);
α y β constantes de integración. Ayuda: considere las ecuaciones de Euler–Lagrange.

\label{ej6}
\item{ \bf }
Son afectadas las ecuaciones de movimiento si al lagrangiano se le agrega una derivada
total con respecto al tiempo?.

\label{ej7}
\item{ \bf }
Hallar la curva de longitud mı́nima que une dos puntos de la superficie de un cilindro.

\label{ej8}
\item{ \bf }
Cuando se arroja un objeto hacia arriba, desde la superficie terrestre, el tiempo que
transcurre hasta que vuelve a tocar tierra resulta tc = 2v0 /g.
\begin{enumerate}[label=(\alph*)]
\item  A partir de una función de prueba dada por:
\[
y(t) = + \sum_{n=1}^\infty an cos(nωt) + \sum_{n=1}^\infty bn sen(nωt)
\]
(desarrollo en serie de Fourier para 0 < t < tc ), con ω = 2π/tc , encuentre
la mejor aproximación a la trayectoria con el criterio del principio de Hamilton.
Utilizando el valor que toma la acción para dicha trayectoria, compare la solución
obtenida con la correspondiente al resultado y(t) = v0 t − 12 gt2 . Qué conclusiones
saca?.
\end{enumerate}

\label{ej9}
\item{ \bf }
Cambia la forma de un lagrangiano cuando se lo escribe en distintas coordenadas
generalizadas?. Muestre un ejemplo.

\label{ej10}
\item{ \bf }
Muestre que el lagrangiano L(q, q̇, q̈, t) = aq̈ + btq̈ + L0 (q, q̇, t) (a y b son constantes
arbitrarias) conduce a ecuaciones de movimiento que son de segundo orden.
